...
dir_save = ['C:\Users\Admin\Desktop\2025 Cavity\' ...
            '2025 Cavity Data Collection\raw_ECG_inference_dataset\'];

dir_gauss = '[wide_gaussian]\';
dir_input = '[input]\';
...
for i=1:length(list_wav_files)
    if contains(list_wav_files(i).name,'[P01EXTRA]')
        idx_to_delete(end+1) = i;
    end
end

list_wav_files(idx_to_delete) = [];
...
for index = 1:size(list_wav_files, 1)
    ...
    %ECG is extracted in 6 sec windows first, because this is 
    %required for the gaussian generation algorithm 
    ECG_6s  = ECG(floor(fs*(xlim_start-1)) : floor((fs*(xlim_end+1))));
    
    HS_4s = HS_filt(floor(fs*xlim_start) : floor(fs*xlim_end));
    HS_4s = HS_4s / max(abs(HS_4s));

    % extracting gaussian waveform and r peak positions from ecg
    [ecg_4sec, ~, ecg_gaussian, hr_pos]= gaussian_generation(ECG_6s, fs);
    ecg_4sec = ecg_4sec / max(abs(ecg_4sec));
    ...
    save_ground_truth(dir_gaussian, ecg_4sec);
    end
    end
