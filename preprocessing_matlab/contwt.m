...
%----------------------------------------------------------------------------
function [wave,period,scale,coi, dj, paramout, k, filter_h] = ...
...
for a1 = 1:J1+1
	[daughter(a1,:),fourier_factor,coi,dofmin, paramout]=wave_bases(mother,k,scale(a1),param);	
    freq_k = 1 / (fourier_factor * scale(a1));
    if freq_k>25
        if freq_k<50
            wave(a1,:) = ifft(f.*daughter(a1,:));  % wavelet transform[Eqn(4)]
        end
    end
% 	wave(a1,:) = ifft(f.*daughter(a1,:));  % wavelet transform[Eqn(4)]
end

period = 1 ./ (fourier_factor * scale);
