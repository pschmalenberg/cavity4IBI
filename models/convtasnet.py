        ...
        for r in range(R):
            for x in range(X):
                self.separation.append(
                    Conv1D_Block(B, H, P, 2**x, norm, causal, skip_con)
                )
        self.skip_con = skip_con
...
# class Speech_ConvTasNet(nn.Module):
#     def __init__(self, checkpoint):
#         super(Speech_ConvTasNet, self).__init__()
#         self.model = models.ConvTasNet.from_pretrained(checkpoint)
...
class Adversary(nn.Module):
    ...
    def forward(self, embed):
        """
        embed: (batch_size, num_features)
        returns: (batch_size)
        """
        embed = embed.mean(dim=-1)
        grl = self.GRL(embed)
        out = self.relu(self.linear1(grl))
        out = torch.sigmoid(self.linear2(out))
        return out.squeeze()


class GRL_ConvTasNet(nn.Module):
    def __init__(self, n_classes):
        super(GRL_ConvTasNet, self).__init__()
        self.model = ConvTasNet(return_embed=True)
        self.grl = Adversary(128, n_classes)

    def forward(self, x):
        pred, embed = self.model(x)
        return pred, self.grl(embed)


if __name__ == "__main__":
    pretrained = "JorisCos/ConvTasNet_Libri2Mix_sepnoisy_8k"
    x = torch.randn(8, 32000) / 10
    nnet = ConvTasNet()
    s = nnet(x)
    print(x, s[:, 0])
